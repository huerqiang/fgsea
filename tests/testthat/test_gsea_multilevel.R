...
test_that("fgseaMultilevel gives valid P-value for 5990980_Cell_Cycle", {
    ...
    set.seed(42)
    fgseaMRes <- fgseaMultilevel(example.pathway, ranks, eps_dd = 0.0)
    pval <- fgseaMRes$pval
    expect_true(1e-29 <= pval && pval <= 1e-25)
})

test_that("The eps_dd parameter works correctly with 5990980_Cell_Cycle", {
    data(examplePathways)
    data(exampleRanks)
    ranks <- sort(exampleRanks, decreasing = TRUE)
    example.pathway <- examplePathways["5990980_Cell_Cycle"]
    ...
    set.seed(42)
    expect_warning(fgseaMRes <- fgseaMultilevel(example.pathway, ranks, eps_dd = 1e-10))
...
test_that("The eps_dd parameter works correctly with 5991504_Extension_of_Telomeres", {
    data(examplePathways)
    data(exampleRanks)
...
test_that("The `eps_dd` parameter works correct in fgseaMultilevelCpp", {
    data(exampleRanks)
    ranks <- sort(exampleRanks, decreasing = TRUE)
    pvalue <- fgsea:::fgseaMultilevelCpp(enrichmentScores = 0.95, ranks = ranks, pathwaySize = 50,
                                         sampleSize = 501, seed = 42, eps_dd = 1e-5, sign = FALSE)
...
test_that("fgsea throws a warning when reaching eps_dd", {
    data(examplePathways)
    data(exampleRanks)
