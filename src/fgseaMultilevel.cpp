...
DataFrame fgseaMultilevelCpp(const NumericVector& enrichmentScores,
                             ...
                             int sampleSize, int seed,double eps_dd, bool sign)
{
    ...
    if (maxES >= 0){
        esRulerPos.extend(abs(maxES), seed, eps_dd);
    }
    if (minES < 0){
        esRulerNeg.extend(abs(minES), seed, eps_dd);
    }
    ...
    for (int i = 0; i < nrow; i++){
        ...
        if (currentES >= 0.0){
            resPair = esRulerPos.getPvalue(abs(currentES), eps_dd, sign);
            pvalRes.push_back(resPair.first);
            isCpGeHalf.push_back(resPair.second);
        }
        else{
            resPair = esRulerNeg.getPvalue(abs(currentES), eps_dd, sign);
            pvalRes.push_back(resPair.first);
            isCpGeHalf.push_back(resPair.second);
        }
    }
